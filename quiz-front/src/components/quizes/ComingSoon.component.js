...
export default class ComingSoon extends Component {
    constructor(props) {
        ...
        this.state = {comingSoon: [], isAdmin: false}
    }
    ...
    async componentDidMount() {
        await axios.get("/quizes?isPublic=true&api_key="+process.env.REACT_APP_API_KEY)
            .then(res => {
                let temp = res.data.quizes.filter(q => q.quizStartDate > new Date().toISOString());
    ...
    comingSoonList() {
        ...
        return this.state.comingSoon.map((currQuiz, index) => {
    ...
    async onClickQuizRemove(id) {
        ...
        axios.get(`/quizes?page=${cp}&isPublic=true&api_key=${process.env.REACT_APP_API_KEY}`)
            .then(res => {
                ...
                this.setState({comingSoon: temp, pageCount: res.data.pageCount});
            })
        .catch(err => err);
    }
    ...
    async previousPage(e) {
        ...
        await axios.get(`/quizes?page=${counter}&isPublic=true&api_key=${process.env.REACT_APP_API_KEY}`)
            .then(res => {
                ...
                this.setState({comingSoon: temp, page: counter});
            })
        .catch(err => err);

        window.history.replaceState("/quizes/coming-soon/", "Quiz OKI", `/quizes/coming-soon?page=${counter}`);
    }
