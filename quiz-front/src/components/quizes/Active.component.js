...
export default class Active extends Component {
    ...
    async componentDidMount() {
        ...
        await axios.get(`/quizes?page=${cp}&isPublic=true&api_key=${process.env.REACT_APP_API_KEY}`)
            .then(res => {
                let temp = res.data.quizes.filter(q => q.quizStartDate <= new Date().toISOString() && q.quizEndDate >= new Date().toISOString());
                if(temp.length > 1) temp.sort((a, b) => a.lastTake > b.lastTake ? -1 : 1);
    ...
    activeList() {
        return this.state.active.map((currQuiz, index) => {
    ...
    async previousPage(e) {
        ...
        window.history.replaceState("/quizes/active/", "Quiz OKI", `/quizes/active?page=${counter}`);
    }
