...
const Result = props => {
    if(props.state.renderDialog && props.state.quiz.quizStartDate <= new Date().toISOString()) {
        return(
            <div className="text-light p-3 mt-4 d-flex justify-content-between align-items-center question-section" 
                style={{border: "none", backgroundColor: "#333444", boxShadow: "2px 2px 2px black"}}
            >
                <div>
                ...
                <div className="btn btn-success">
                    <Link className="text-dark" to={"/ranking/"+props.id}>Zobacz ranking</Link>
                </div>
            </div>
        );
    } if(props.state.renderDialog && props.state.quiz.quizStartDate > new Date().toISOString()) {
